...
public class VariableExample2 {
	public static void main(String[] args) {
		...
		// 자료형 변수명 = 리터럴;
		// 프로그래밍에서는 대입되는 데이터를 리터럴이라는 단어로도 표현한다.
		...
		// 실수형
		float floatNumber = 1.2345f; // 대소문자 상관 없음. f 안 붙이면 double로 인식
		 ...
		 *   'B' 문자 그대로가 대입되면 변수에 숫자 66으로 변환돼서 저장됨
		...
		// 변수 명명 규칙
		// 1. 대소문자 구분 O, 길이 제한 X
		int jsdkljflksdjflksjklfjdlkfjlskdjfsldkjf;
		int jsdkljflksdjflksjklfjdlkfjlskdjfsldkjF;
		// 서로 다른 변수 (대소문자를 구분하기 때문에)
		...
		int $intNumber; // 문제는 없지만 개발자가 직접 사용 안 함
		int int_number; // 문제는 없지만 자바는 카멜표기법 쓰기 때문에 _ 작성 안 함
		                // _ 작성표기법은 DB에서 사용
		...
		// 6. 변수명은 언어를 가리지 않는다. (하지만 사용하지 않음. 보통 영어로 쓴다.)
		int 정수1 = 10;
		int aa;
		 ...
		 * - 자료형 앞에 final 키워드를 작성(마지막에 대입되는 값이라는 뜻)
		 * 
		 * - 상수 명명 규칙: 모두 대문자, 여러 단어 작성 시 _(언더바) 사용
		 * 
		 * - 상수를 사용하는 경우
